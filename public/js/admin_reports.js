...
function refreshReports() {
  ...
  for (var i = 0; i < reports.length; i++) {
    ...
    username.appendChild(
      document.createTextNode(reports[i].id + ": " + reports[i].username),
    );
    description = document.createTextNode(reports[i].description);
    console.log(reports[i]);

    username.innerHTML +=
      " <button onclick='deleteReport(" + reports[i].id + ")'>X</button>";
...
async function init() {
  ...
  if (!status.admin) {
    ...
    content.innerHTML =
      "<h1 align='center'>401, you're a cunt</h1> <hr> <p align='center'> try better next time </p>";
    return;
  }
  await getReports();
  refreshReports();
