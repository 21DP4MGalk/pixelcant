...
function updateUserMessage(username, admin, banned) {
  ...
  if (!admin && !banned) {
    userMessage.innerText =
      "Not banned yet, an impressive achievement! \nKeep it up, or else.";
  } else if (banned && !admin) {
    userMessage.innerText =
      "Wether you drew swastikas or \nadvocated for raisin enjoyer genocide, \nthe ban was probably earned. \nUnless we did \nit for fun.\nFun fact: there's no way to get unbanned";
  } else if (banned && admin) {
    userMessage.innerText =
      "You cheeky debugging bastard, enjoy this clipping text, I'm not placing newlines, you're not my dad";
  } else if (!banned && admin) {
    userMessage.innerText =
      "One of our famously GOODtm \nadministrators, the backbone of the \nsystem and the fabric of this \nlocalized reality. \nStill bald though";
  }
}
